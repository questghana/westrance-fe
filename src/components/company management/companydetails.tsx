...
//   {
//     name: "Company Type:",
//     detail: "LLC (Limited Liability Company)",
//   },
//   {
//     name: "Industry:",
...
//   {
//     name: "Address:",
//     detail: "14 Independence Avenue, Ridge",
//   },
//   {
//     name: "Website:",
//     detail: "unitechllc.com",
//   },
// ];
// const conpamnyDetails3 = [
//   {
//     name: "Administrative Full Name:",
//     detail: "Nana Kwame Asare",
//   },
//   {
//     name: "Administrative Email Address:",
//     detail: "kwame.asare@accsoftgh.com",
//   },
// ];

export const CompanyDetails: FC<CompanyManagementHeaderProps> = ({ ...props }) => {
  ...
  const handleDelete = async () => {
    try {
      setLoading(true)
      const response = await axios.delete(`/admin/deleteCompany/${companyId}`)
      toast.success(response.data.message)
      props.goToStep("company management")
    } catch (error: any) {
  ...
  const updateCompanyStatus = async (companyId: string | undefined, status: string) => {
    try {
      const response = await axios.patch(`/admin/Active-Deactive-Company/${companyId}/status`, { status })
      toast.success(response.data.message)
    ...
    >
      <Flex className="justify-between max-md:flex-col items-start">
        <TabsList className="bg-[#ebf2fd]">
          ...
          <TabsTrigger value="Employees">Company Employees</TabsTrigger>
        </TabsList>
          ...
          <Flex>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="p-1">
                <DropdownMenuItem className="p-2" onClick={() => updateCompanyStatus(companyId, "Active")}>
                  Active
                </DropdownMenuItem>
                <DropdownMenuItem className="p-2" onClick={() => updateCompanyStatus(companyId, "Deactive")}>
                  Deactive
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
              ...
              <Flex className="justify-between mt-6 max-sm:flex-col-reverse gap-0 items-start">
                ...
                <Center className="flex-col">
                  <img
                    ...
                    height={90}
                  />
                  <h1 className="text-gray-400">Company Logo</h1>
                </Center>
              </Flex>
