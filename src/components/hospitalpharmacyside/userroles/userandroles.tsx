...
export const UsersAndRolesHospital: FC<stylingProps> = ({ margin, padding }) => {
  ...
  useEffect(() => {
    const getHospitalEmployees = async () => {
      try {
        setLoading(true);
        const response = await axios.get("/hospital/employees");
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      setLoading(true)
      const response = await axios.post("/hospital/addEmployeeRoles", payload);
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      <SelectTrigger className="lg:w-70 md:w-96 py-6 bg-[#F8F8F8]">
