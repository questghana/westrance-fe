...
import Empaddmodal from "../modal/empaddmodal";
...
interface ReusableAddEmployeeProps {
  onGoToList: () => void;
}

export const ReusableAddEmployee = ({ onGoToList }: ReusableAddEmployeeProps) => {
  ...
  const onSubmit = async (data: AddEmployeeForm) => {
    ...
    try {
      if (mode === "add") {
        ...
        const employeeId = response.data.data.employee.employeeId
        useAuthStore.getState().setnewEmployeeId(employeeId)
        setCopytxt(employeeId)
        if (response.data) {
          setOpen(true);
        }
      }
        ...
        const response = await axios.put('/company/edit/employee', payload)
  ...
  useEffect(() => {
    ...
    console.log("Errors:", selectedEmployee?.employeeId);
                ...
                <Avatar className="w-[220px] h-[220px] bg-[#FBFBFB] rounded-lg px-6 py-6 border-2 border-dashed">
                  {previewUrl ? (
                    ...
                    <AvatarFallback className="flex items-end overflow-hidden">
                      <img
                        src={selectedEmployee?.profileImage || profileicon}
                        alt="profileicon"
                        className="h-35"
                      />
                    </AvatarFallback>
